...
export class PacketLossTestTool extends BaseTool {
  ...
  protected validateArguments(args: Record<string, unknown>): void {
    ...
    if (args.packetCount !== undefined) {
      if (typeof args.packetCount !== 'number' || args.packetCount < 10 || args.packetCount > 1000) {
        throw new Error('Packet count must be a number between 10 and 1000');
      }
    }

    if (args.batchSize !== undefined) {
      if (typeof args.batchSize !== 'number' || args.batchSize < 1 || args.batchSize > 50) {
        throw new Error('Batch size must be a number between 1 and 50');
      }
    }

    if (args.batchWaitTime !== undefined) {
      if (typeof args.batchWaitTime !== 'number' || 
          args.batchWaitTime < 100 || 
          args.batchWaitTime > 5000) {
        throw new Error('Batch wait time must be between 100 and 5000 milliseconds');
      }
    }
  ...
  protected async executeImpl(
    ...
    try {
      ...
      const lostPackets = Math.round((packetLossPercentage / 100) * totalPackets);

      // Generate batch results (simplified simulation)
      const batchResults = this.generateBatchResults(numBatches, batchSize, lostPackets);
      ...
      return this.createToolResult(true, packetLossData, undefined, executionTime);
  ...
  private extractPacketLossOptions(args: Record<string, unknown>): PacketLossTestOptions {
    return {
  ...
  private createPacketLossMeasurements(options: PacketLossTestOptions): SpeedTestMeasurement[] {
