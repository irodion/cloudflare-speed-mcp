...
export class ConnectionInfoTool extends BaseTool {
  ...
  protected validateArguments(args: Record<string, unknown>): void {
    if (args.includeLocation !== undefined && typeof args.includeLocation !== 'boolean') {
  ...
  protected async executeImpl(
    ...
    try {
      ...
      return this.createToolResult(true, connectionData, undefined, executionTime);
  ...
  private extractConnectionInfoOptions(args: Record<string, unknown>): ConnectionInfoOptions {
    return {
  ...
  private async getConnectionDetails(options: ConnectionInfoOptions): Promise<ConnectionInfoResult['data']> {
