...
describe('CloudflareSpeedTestClient', () => {
  ...
  describe('runSpeedTest', () => {
    ...
    it('should handle speed test errors', async () => {
      ...
      await expect(client.runSpeedTest()).rejects.toThrow('Speed test execution failed');
    });
    ...
    it('should handle timeout errors', async () => {
      ...
      } as unknown as ConstructorParameters<typeof CloudflareSpeedTestClient>[0]);

      await expect(shortTimeoutClient.runSpeedTest()).rejects.toThrow('timed out');
    });
    ...
    it('should filter measurements by test type', async () => {
      ...
      const SpeedTest = require('@cloudflare/speedtest').default;
      const lastCall = jest.mocked(SpeedTest).mock.calls[jest.mocked(SpeedTest).mock.calls.length - 1];
  ...
  describe('getConnectionInfo', () => {
    ...
    it('should handle connection info API errors', async () => {
      ...
      await expect(client.getConnectionInfo()).rejects.toThrow('Connection info request failed');
    });

    it('should handle network errors', async () => {
      mockFetch.mockRejectedValueOnce(new Error('Network error'));

      await expect(client.getConnectionInfo()).rejects.toThrow('Failed to get connection info');
    });
  });
  ...
  describe('discoverServers', () => {
    ...
    it('should handle server discovery API errors', async () => {
      ...
      await expect(client.discoverServers()).rejects.toThrow('Server discovery failed');
    });
  });
  ...
  describe('rate limiting', () => {
    it('should enforce speed test rate limits', async () => {
      const rateLimitedClient = new CloudflareSpeedTestClient({
        rateLimits: { REQUESTS_PER_MINUTE: 10, SPEED_TESTS_PER_HOUR: 1, BURST_LIMIT: 3 }
      } as unknown as ConstructorParameters<typeof CloudflareSpeedTestClient>[0]);
      ...
      await expect(rateLimitedClient.runSpeedTest()).rejects.toThrow('Rate limit exceeded');
    });
  });
});